...
void ReadyQueueToIsRunning(CPU_p cpu) {
    if (!isEmptyPriorityQueue(cpu->readyQueue)) {
        ...
        // MT
        cpu->isRunning = pcb;
        //printf("isRUnning = pcb");
    } else {
        cpu->isRunning = cpu->idle;
        //printf("isRUnning = idle");
    }
}
...
int threadConditionWait(PCB_p thePCB, CPU_p cpu, condition_type theCondition, Mutex_p theMutex) {
    int answer = 0;
    unLock(theMutex, thePCB, cpu->readyQueue);
    enqueue(cpu->threadConditionArray[thePCB->prodcon_num][theCondition], thePCB);
    ReadyQueueToIsRunning(cpu);
    return answer;
}

void threadConditionSignal(PCB_p thePCB, CPU_p cpu, condition_type theCondition) {
    while (!isEmpty(cpu->threadConditionArray[thePCB->prodcon_num][theCondition])) {
        enqueue_priority(cpu->readyQueue, dequeue(cpu->threadConditionArray[thePCB->prodcon_num][theCondition]));
    }
}

//this is the basic producer consumer
int processProdConOld(PCB_p thePCB, CPU_p cpu) {
    int switchedToNewPCB = 0;
    //printf("processProdCon ");
    //printf("pid: %d PC: %u ", thePCB->pid, thePCB->PC);
    ...
    if(thePCB->isProducer) {
            ...
            if (*cpu->signalMutexArray[thePCB->prodcon_num]->theData == 1) {
                ...
                if (Lock(cpu->dataMutexArray[thePCB->prodcon_num], thePCB, cpu->outfile)) {                    
                    ...
                    *cpu->signalMutexArray[thePCB->prodcon_num]->theData = 0;
                    ...
                    unLock(cpu->dataMutexArray[thePCB->prodcon_num], thePCB, cpu->readyQueue);
                    unLock(cpu->signalMutexArray[thePCB->prodcon_num], thePCB, cpu->readyQueue);
                    threadConditionSignal(thePCB, cpu, ready_to_read);
                } else {
                    //you didn't get the lock so you are in the dataMutex waiting queue
                    //but you also have a lock on the signalMutex
                    ReadyQueueToIsRunning(cpu);
                ...
                //printf("signal = 0 for producer");
                //if the signal is 0, release the mutex for signal
                //you got a lock on the signal and found that you need to wait for the consumer
                threadConditionWait(thePCB, cpu, ready_to_write, cpu->signalMutexArray[thePCB->prodcon_num]);
                //unLock(cpu->signalMutexArray[thePCB->prodcon_num], thePCB, cpu->readyQueue);
            }
        } else {
            //you didn't get the lock so you are in the signalMutex waiting queue
        ...
        if (Lock(cpu->signalMutexArray[thePCB->prodcon_num], thePCB, cpu->outfile)) {
            ...
            if (*cpu->signalMutexArray[thePCB->prodcon_num]->theData == 0) {
                ...
                //printf("signal = 1 for consumer");
                //if the signal is 1, release the mutex for signal
                //you got a lock on the signal and found that you need to wait for the producer
                threadConditionWait(thePCB, cpu, ready_to_read, cpu->signalMutexArray[thePCB->prodcon_num]);
                //unLock(cpu->signalMutexArray[thePCB->prodcon_num], thePCB, cpu->readyQueue);
            }
        } else {
            //you didn't get the lock so you are in the signalMutex waiting queue
...
void run(CPU_p cpu) {
    ...
    int i;
    int j;
    for (j = 0; j < cpu->num_prodcons;j++) {
        cpu->threadConditionArray[j] = malloc(enum_counter * sizeof(fifo_queue_p));
        for (i = 0; i < enum_counter; i++) {
            cpu->threadConditionArray[j][i] = create_queue();
        }
    }
